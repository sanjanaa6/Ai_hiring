...
class Judge0Service {
  ...
  async submitCode(code, language, input = '', expectedOutput = '') {
    try {
      ...
      console.log('✅ [JUDGE0] Submission created:', response.data.token);
      ...
      console.error('❌ [JUDGE0] Submit error:', error.message);
      return {
        success: false,
        error: error.message
      };
  ...
  async runTestCases(code, language, testCases) {
    try {
      ...
      console.error('❌ [JUDGE0] Test cases error:', error.message);
      return {
        success: false,
        error: error.message,
        results: []
